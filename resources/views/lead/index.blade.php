<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                        ...
                        <!-- Modal body -->
                        <form class="p-4 md:p-5" method="POST" action="{{ route('lead.store') }}">
                            @csrf
                            <div class="grid gap-4 mb-4 grid-cols-2">
                                ...
                                <div class="mb-4 col-span-2">
                                    ...
                                    <input type="text" id="address" name="address" class="w-full border border-gray-300 rounded p-2" required>
                                </div>
                                <div class="mb-4 col-span-2">
                        ...
                        <tbody>
                            @foreach ($leads as $index => $lead)
                                <tr class="bg-white border-b dark:bg-white-800 dark:border-white-700 border-white-200">
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ $lead->nama }}</td>
                                    <td class="px-6 py-4">{{ $lead->email }}</td>
                                    <td class="px-6 py-4">{{ $lead->alamat }}</td>
                                    <td class="px-6 py-4">
                                        @php
                                            $statusColors = [
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-2">
                                            <button
                                                ...
                                                onclick="openEditModal({{ $lead->id }}, '{{ $lead->nama }}', '{{ $lead->email }}', '{{ $lead->alamat }}', '{{ $lead->status }}')"
                                                data-modal-target="edit-modal" data-modal-toggle="edit-modal">
                                                Edit
                                            </button>
                                            <form method="POST" action="{{ route('lead.destroy', $lead->id) }}" onsubmit="return confirm('Yakin ingin menghapus lead ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                                                    onclick="confirmDelete('{{ route('lead.destroy', $lead->id) }}')"
            ...
            <!-- Modal Edit Calon Customer -->
            <div id="edit-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <!-- Header -->
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Calon Customer</h3>
                        ...
                        <form id="edit-form" method="POST">
                            ...
                            <div class="p-4 md:p-5">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="email" id="edit-email" name="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                                </div>
                                <div class="mb-4">
                                    <label for="edit-address" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Alamat</label>
                                    <input type="text" id="edit-address" name="address" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                                </div>
                                <div class="mb-4">
                                    <label for="edit-status" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status</label>
                                    <select id="edit-status" name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                ...
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                        ...
                        <div class="p-4 md:p-5 text-center">
                            ...
                            <h3 class="mb-5 text-lg font-normal text-gray-600 dark:text-gray-400">Apakah anda yakin ingin menghapus lead ini?</h3>
